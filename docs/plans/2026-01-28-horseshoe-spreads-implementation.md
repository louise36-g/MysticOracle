# Horseshoe Spreads Implementation Plan

> **For Claude:** Use superpowers:subagent-driven-development to implement this plan task-by-task.

**Goal:** Implement 20 themed Horseshoe (7-card) spread layouts with accordion UI, following the existing 5-card pattern.

**Architecture:** Create `horseshoeLayouts.ts` mirroring `fiveCardLayouts.ts` structure, then extend `SpreadIntroSelector.tsx` to handle `SpreadType.HORSESHOE`.

**Tech Stack:** TypeScript, React, Framer Motion, Tailwind CSS, Lucide icons

---

## Task 1: Create horseshoeLayouts.ts - Type Definitions

**Files:**
- Create: `constants/horseshoeLayouts.ts`
...
```bash
git add constants/horseshoeLayouts.ts
git commit -m "feat(horseshoe): add type definitions for horseshoe layouts"
```

---

## Task 2: Add Love & Relationships Layouts

**Files:**
- Modify: `constants/horseshoeLayouts.ts`

**Step 1: Add layout definitions after interfaces**
...
Run: `npx tsc --noEmit`
Expected: No errors (may have unused variable warning, that's OK)

**Step 3: Commit**

```bash
git add constants/horseshoeLayouts.ts
git commit -m "feat(horseshoe): add Love & Relationships layouts"
```

---

## Task 3: Add Career & Work Layouts

**Files:**
- Modify: `constants/horseshoeLayouts.ts`

**Step 1: Add after Love layouts (inside HORSESHOE_LAYOUTS)**
...
```bash
git add constants/horseshoeLayouts.ts
git commit -m "feat(horseshoe): add Career & Work layouts"
```

---

## Task 4: Add Money & Finances Layouts

**Files:**
- Modify: `constants/horseshoeLayouts.ts`

**Step 1: Add after Career layouts**
...
```bash
git add constants/horseshoeLayouts.ts
git commit -m "feat(horseshoe): add Money & Finances layouts"
```

---

## Task 5: Add Life Path & Major Decisions Layouts

**Files:**
- Modify: `constants/horseshoeLayouts.ts`

**Step 1: Add after Money layouts**
...
```bash
git add constants/horseshoeLayouts.ts
git commit -m "feat(horseshoe): add Life Path & Major Decisions layouts"
```

---

## Task 6: Add Family & Personal Layouts

**Files:**
- Modify: `constants/horseshoeLayouts.ts`

**Step 1: Add after Life Path layouts, then close the HORSESHOE_LAYOUTS object**
...
```bash
git add constants/horseshoeLayouts.ts
git commit -m "feat(horseshoe): add Family & Personal layouts"
```

---

## Task 7: Add Category Configurations

**Files:**
- Modify: `constants/horseshoeLayouts.ts`

**Step 1: Add category configurations after HORSESHOE_LAYOUTS**
...
```bash
git add constants/horseshoeLayouts.ts
git commit -m "feat(horseshoe): add category configurations"
```

---

## Task 8: Add Questions for All Layouts

**Files:**
- Modify: `constants/horseshoeLayouts.ts`

**Step 1: Add questions after categories**
...
export const HORSESHOE_LAYOUT_QUESTIONS: Record<HorseshoeLayoutId, HorseshoeQuestion[]> = {
  ...
  friendships: [
    { id: 'hs_fr1', textEn: 'What am I seeking from this friendship that I could also give myself?', textFr: 'Qu\'est-ce que je cherche dans cette amitié que je pourrais aussi me donner?' },
    { id: 'hs_fr2', textEn: 'How have I changed, and does this friendship still reflect who I am?', textFr: 'Comment ai-je changé, et cette amitié reflète-t-elle encore qui je suis?' },
    { id: 'hs_fr3', textEn: 'What truth about this connection am I avoiding?', textFr: 'Quelle vérité sur cette connexion est-ce que j\'évite?' },
  ],
  difficult_relatives: [
...
```bash
git add constants/horseshoeLayouts.ts
git commit -m "feat(horseshoe): add all questions and helper functions"
```

---

## Task 9: Update SpreadIntroSelector - Add Imports and Colors

**Files:**
- Modify: `components/reading/phases/SpreadIntroSelector.tsx`

**Step 1: Add horseshoe imports after existing imports (around line 49)**
...
**Step 2: Add Coins icon to imports (line 4-19)**

Add `Coins` to the lucide-react import.

**Step 3: Add new color classes (after line 74)**
...
**Step 4: Add Coins to ICON_MAP (after line 64)**
...
git commit -m "feat(horseshoe): add imports and color classes to SpreadIntroSelector"
```

---

## Task 10: Update SpreadIntroSelector - Add Horseshoe Case

**Files:**
- Modify: `components/reading/phases/SpreadIntroSelector.tsx`

**Step 1: Update type unions (around line 77-78)**
...
**Step 2: Add HORSESHOE case to useMemo (after line 159, before default)**
...
**Step 3: Update currentQuestions useMemo to handle horseshoe (after line 178)**

For horseshoe, ALL questions are layout-specific (not category-level), so we need to handle this:
...
```bash
git add components/reading/phases/SpreadIntroSelector.tsx
git commit -m "feat(horseshoe): integrate horseshoe spreads into SpreadIntroSelector"
```
...
**Step 2: Test the horseshoe spread**

1. Navigate to the app
2. Go to "Tarot Readings" dropdown
3. Select "Horseshoe Spreads"
4. Verify:
   - 5 category accordions appear (Love, Career, Money, Life Path, Family)
   - Each category has correct icon and color
   - Clicking a category expands to show 4 layouts
   - Each layout shows label, tagline
   - Clicking "The cards in this layout" shows 7 positions
   - Selecting a layout shows question input
   - Suggested questions appear (3-4 per layout)
   - Clicking a suggested question fills the input

**Step 3: Test each category**

Expand each category and verify layouts load correctly:
- Love & Relationships (rose): 4 layouts
- Career & Work (blue): 4 layouts
- Money & Finances (amber): 4 layouts
- Life Path (purple): 4 layouts
- Family & Personal (teal): 4 layouts

**Step 4: Final commit**

```bash
git add -A
git commit -m "feat(horseshoe): complete horseshoe spreads implementation

- 20 themed layouts across 5 categories
- 63 introspective preformed questions
- Accordion UI matching 5-card pattern
...
- [ ] `constants/horseshoeLayouts.ts` created with all types, layouts, categories, questions
- [ ] `SpreadIntroSelector.tsx` handles `SpreadType.HORSESHOE`
- [ ] New color classes (blue, purple, teal) added
- [ ] Coins icon imported and mapped
- [ ] TypeScript compiles without errors
- [ ] All 5 categories display with correct icons/colors
- [ ] All 20 layouts accessible via accordion
- [ ] 7-card positions display correctly
- [ ] Questions load per-layout (not per-category)
- [ ] French translations display when language is FR
